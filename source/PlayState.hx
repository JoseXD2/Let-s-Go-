...
class PlayState extends MusicBeatState
{
	...
	private var noteSpecialAnim:Bool = false;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'poison':
				dialogue = CoolUtil.coolTextFile(Paths.txt('poison/poisonDialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'blossom':
			{
					...
					var sky:FlxSprite = new FlxSprite(-550, -450).loadGraphic(Paths.image('blossom/3'));
					sky.antialiasing = true;
					sky.scrollFactor.set(0.9, 0.9);
					sky.active = false;
					add(sky);
					
					var lake:FlxSprite = new FlxSprite(-550, -450).loadGraphic(Paths.image('blossom/2'));
					lake.antialiasing = true;
					lake.scrollFactor.set(0.9, 0.9);
					lake.active = false;
					add(lake);
					
					var ground:FlxSprite = new FlxSprite(-550, -450).loadGraphic(Paths.image('blossom/1'));
					...
					FlxG.sound.cache(Paths.sound("shooters"));
			}
			case 'stage':
				{
						defaultCamZoom = 0.9;
		...
		var doof2:DialogueBox = new DialogueBox(false, dialogue2);
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'petals':
					FlxG.sound.playMusic(Paths.music('setting', 'shared'), 0, true);
					FlxG.sound.music.fadeIn(1, 0, 0.8);
					rosieIntro();
				case 'bloom':
					FlxG.sound.playMusic(Paths.music('action', 'shared'), 0, true);
					FlxG.sound.music.fadeIn(1, 0, 0.8);
					rosieIntro();
				case 'poison':
					FlxG.sound.playMusic(Paths.music('action', 'shared'), 0, true);
					FlxG.sound.music.fadeIn(1, 0, 0.8);
					rosieangryIntro(doof);
				default:
					if(doof!=null)
					{
						showDialogue(doof);
	...
	function rosieIntro():Void 
		{
		if (SONG.song.toLowerCase() == 'petals')
			{	
			var cutscene = new Cutscene1( CoolUtil.coolTextFile(Paths.txt('petals/petalsDialogue')) );
			cutscene.cameras = [camHUD];
			add(cutscene);
			cutscene.finishThing = startCountdown;
			}
		
		if (SONG.song.toLowerCase() == 'bloom')
			{	
			var cutscene = new Cutscene2( CoolUtil.coolTextFile(Paths.txt('bloom/bloomDialogue')) );
			cutscene.cameras = [camHUD];
			add(cutscene);
			cutscene.finishThing = startCountdown;
			}
	...
	function rosieangryIntro(?dialogueBox:DialogueBox):Void {
		if (dialogueBox != null)
			{
				add(dialogueBox);
			}
	...
	function fuhthishi(?dialogueBox:DialogueBox):Void {
		inCutscene = true;
		...
		//i know, i know, my coding's trash
		new FlxTimer().start(0.1, function(tmr:FlxTimer){
		dad.playAnim('fuck this');
		});
		
		new FlxTimer().start(1.2, function(tmr:FlxTimer){	
		FlxG.sound.play(Paths.sound('micThrow', 'shared'));
		});

		new FlxTimer().start(1.8, function(tmr:FlxTimer){
			if (dad.curCharacter == 'rosie-angry' && dad.animation.curAnim.name.startsWith('fuck this'))
				{
	...
	function defeat():Void 
		{
			...
			remove(boyfriend);
			remove(gf);
			FlxG.sound.playMusic(Paths.music('calm', 'shared'), 0);
			FlxG.sound.music.fadeIn(1, 0, 0.8);	
		if (SONG.song.toLowerCase() == 'poison')
			{
			var cutscene = new Cutscene3( CoolUtil.coolTextFile(Paths.txt('poison/poisonEndDialogue')) );
			cutscene.cameras = [camHUD];
			add(cutscene);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (Std.isOfType(songNotes[3], Bool) && gottaHitNote)
					noteSpecialAnim = true;
				else
					noteSpecialAnim = false;

				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, noteSpecialAnim);
				swagNote.sustainLength = songNotes[2];
				swagNote.specialAnim = false;
				swagNote.scrollFactor.set(0, 0);
				if (Std.isOfType(songNotes[3], Bool))
					{
						swagNote.specialAnim = true;
					}
					else
					{
						swagNote.specialAnim = false;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (SONG.noteStyle)
			{
				...
				default:
					babyArrow.frames = Paths.getSparrowAtlas('WeekFunkyNOTE_assets');
					babyArrow.animation.addByPrefix('green', 'arrowUP');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
					babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
					babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			{
				case 0:
					boyfriend.playAnim('singLEFTmiss' + altAnim, true);
				case 1:
					boyfriend.playAnim('singDOWNmiss' + altAnim, true);
				case 2:
					boyfriend.playAnim('singUPmiss' + altAnim, true);
				case 3:
					boyfriend.playAnim('singRIGHTmiss' + altAnim, true);
			}
	...
	*/
	...
	override function beatHit()
	{
		...
		//Rosie's ara ara :p
		if (curBeat == 13 && curSong == 'Petals')
			{
				dad.playAnim('ara');
			}
		else if (curBeat == 172 && curSong == 'Petals')
			{
				dad.playAnim('ara');
			}
		
		if (curStep == 952 && curSong == 'Poison')
			{
				boyfriend.playAnim('hey', true);
				gf.playAnim('cheer', true);
			}
